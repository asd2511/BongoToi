...
namespace morekeys
{
	...
	int acce[MAX_KEYS_NUM];
	sf::Sprite acce_press[MAX_KEYS_NUM];
	int meme[MAX_KEYS_NUM];
	sf::Sprite meme_press[MAX_KEYS_NUM];
	...
	bool init()
	{
		...
		if (!atextex.loadFromFile("img/arm.png"))
		{
		...
		for (int i = 0; i < MAX_KEYS_NUM; i++)
		{
			...
			keybord_press[i].setTexture(data::load_texture2("img/keybord/" + std::to_string(i) + ".png"));
			hand[i]= data::cfg["morekeys"]["hand"][i].asInt();
			hand_press[i].setTexture(data::load_texture2("img/hand/" + std::to_string(i) + ".png"));
			face[i] = data::cfg["morekeys"]["face"][i].asInt();
			face_press[i].setTexture(data::load_texture2("img/face/" + std::to_string(i) + ".png"));
			acce[i] = data::cfg["morekeys"]["acce"][i].asInt();
			acce_press[i].setTexture(data::load_texture2("img/acce/" + std::to_string(i) + ".png"));
			meme[i] = data::cfg["morekeys"]["meme"][i].asInt();
			meme_press[i].setTexture(data::load_texture2("img/meme/" + std::to_string(i) + ".png"));

			sounds[i] = data::cfg["morekeys"]["sounds"][i].asInt();
			if (!soundBuffer[i].loadFromFile(("img/sound/" + std::to_string(i) + ".wav")))
				if (!soundBuffer[i].loadFromFile(("img/sound/" + std::to_string(i) + ".ogg")))
					if (!soundBuffer[i].loadFromFile(("img/sound/" + std::to_string(i) + ".flac")));
		...
		// importing sprites
		up.setTexture(data::load_texture("img/up.png"));

		if (is_mouse)
		{
			bg.setTexture(data::load_texture("img/mousebg.png"));
			device.setTexture(data::load_texture("img/mouse.png"), true);
			mouse_left.setTexture(data::load_texture("img/mouse_left.png"), true);
			mouse_right.setTexture(data::load_texture("img/mouse_right.png"), true);
		}
		else
		{
			bg.setTexture(data::load_texture("img/tabletbg.png"));
			device.setTexture(data::load_texture("img/tablet.png"), true);
			mouse_left.setTexture(data::load_texture("img/tablet_left.png"), true);
			mouse_right.setTexture(data::load_texture("img/tablet_right.png"), true);
		}
	...
	void draw()
	{
		...
		int oof = 6;
		std::vector<double> pss = { 210, 475 }; //{ 211.0, 159.0 };
		double dist = hypot(pss[0] - x, pss[1] - y);
		double centreleft0 = pss[0] - 0.7237 * dist / 2;
		double centreleft1 = pss[1] + 0.69 * dist / 2;
		for (double i = 1; i < oof; i++)
		{
			double p0;
			double p1;
			std::vector<double> bez = { pss[0], pss[1], centreleft0, centreleft1, x, y };
	...
	/*	sf::VertexArray fill(sf::TriangleStrip, 26);
		...
		*/
		...
		if (is_emoticonkeep) {
			if (acce_state != -1) {
				window.draw(acce_press[acce_state]);
			}
	...
	void drawDmkReaction(int i) 
	{
		int len = sizeof( meme_press);
		if ((0 <= i - 1) && (i - 1 <= len))
		{
			window.draw(meme_press[i - 1]);
		}
	}
